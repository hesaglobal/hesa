...
export class QueriesComponent implements OnInit {
  ...
  getTarnscribedFiles(page){
    ...
    this.dataServices.callAPI("get", {},`client/customercare/queries?currentPage=${page}&pageSize=${this.pageSize}`).pipe(
      take(1),
      map(res => {
  ...
  viewFile(fileId){
    this.router.navigate(['/client/customercare/audio','queries', fileId]);
  }
  download(file) {
  ...
  updateStatus(event,fileId){
    ...
    if(value&&value.trim()!==''){
      ...
      this.dataServices.callAPI("patch", body,`client/customercare/update/${fileId}`).pipe(
        take(1),
        map(res => {
          if(res.status){
