...
export class ContentComponent implements OnInit {
  ...
  getTranscriptedText(){
      this.dataservice.callAPI("get", {},`client/grievances/content/${this.fileId}`).pipe(
        take(1),
        map(res => {
  ...
  onLanguageChange(event) {
    ...
    if(lang){
      this.dataservice.callAPI("post", {},`client/grievances/translate/${this.fileId}?lang=${lang}`).pipe(
        take(1),
        map(res => {
  ...
  updateResult = (data) => {
    ...
    return this.dataservice.callAPI("patch", data, `client/grievances/update/${this.fileId}`).pipe(
      take(1),
      map(res => {
  ...
  onSubmit() {
    ...
    this.dataservice.callAPI("post", body, `client/grievances/updateContent/${this.fileId}`).pipe(
      take(1),
      map(res => {
        this.alert.toast(res['message'])
        if(this.subPart=='transcribe'){
          this.router.navigate(["/client/publicgrievance/transcribe"])
        }else{
          this.router.navigate(["/client/publicgrievance/grievances"])
        }
      })).subscribe()
  }
}
