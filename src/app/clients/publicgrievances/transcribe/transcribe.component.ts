...
export class TranscribeComponent implements OnInit {
  ...
  getTarnscribedFiles(page){
   ...
   this.dataServices.callAPI("get", {},`client/grievances?currentPage=${page}&pageSize=${this.pageSize}`).pipe(
      take(1),
      map(res => {
  ...
  onProcess(fileKey,fileId){
    ...
    if(this.coinsLeft>0){
      ...
      this.dataServices.callAPI("post", body,`client/grievances/transcription`).pipe(
        take(1),
        map(res => {
          this.getTarnscribedFiles(this.page);
        })).subscribe()
  ...
  onDelete(fileId,fileKey,name){
    this.ask.confirmThis(`Are you sure to delete file <b>${name}</b>?`, () => {
      ...
      this.dataServices.callAPI("delete", body,`client/grievances/deleteGrievance`).pipe(
        take(1),
        map(res => {
  ...
  viewFile(fileId){
    this.router.navigate(['/client/publicgrievance/audio','transcribe', fileId]);
  }
  download(file) {
