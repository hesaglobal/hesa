...
export class TranscribeComponent implements OnInit {
  ...
  getTarnscribedFiles(page){
    ...
    this.dataServices.callAPI("get", {},`client/jobs?currentPage=${page}&pageSize=${this.pageSize}`).pipe(
      take(1),
      map(res => {
  ...
  onProcess(fileKey,fileId){
    ...
    if(this.coinsLeft>0){
      this.fileId=fileId
      const body={fileKey,fileId,module:"Jobs",alreadyUploaded:true}
      this.dataServices.callAPI("post", body,`client/jobs/transcribe/audio`).pipe(
        take(1),
        map(res => {
          this.getTarnscribedFiles(this.page);
        })).subscribe()
  ...
  onDelete(fileId,fileKey,name){
    this.ask.confirmThis(`Are you sure to delete file <b>${name}</b>?`, () => {
      ...
      this.dataServices.callAPI("delete", body,`client/jobs/delete`).pipe(
        take(1),
        map(res => {
