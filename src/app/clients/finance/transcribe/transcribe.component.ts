...
export class TranscribeComponent implements OnInit {
  ...
  getTarnscribedFiles(page){
    ...
    this.dataServices.callAPI("get", {},`client/finance?currentPage=${page}&pageSize=${this.pageSize}`).pipe(
      take(1),
      map(res => {
  ...
  onProcess(fileKey,fileId){
    ...
    if(this.coinsLeft>0){
      this.fileId=fileId
      const body={fileKey,fileId,module:'Finance'}
      this.dataServices.callAPI("post", body,`client/transcribed/audio`).pipe(
        take(1),
        map(res => {
          this.getTarnscribedFiles(this.page);
        })).subscribe()
  ...
  onDelete(fileId:any,fileKey:any,name:any){
    this.ask.confirmThis(`Are you sure to delete this file <b>${name}</b>`, () => {
      this.mode='Delete';
      this.fileId=fileId
      const body={fileId,fileKey,module:'Finance'}
  ...
  onDevProcess(fileKey,fileId){
    if(this.coinsLeft>0){
      ...
      this.dataServices.callAPI("post", body,`client/transcribed/content`).pipe(
        take(1),
        map(res => {
          this.getTarnscribedFiles(this.page);
        })).subscribe()
  ...
  viewFile(fileId){
    this.router.navigate(['/client/finance/audio','transcribe', fileId]);
  }
  download(file) {
