...
export class QueriesComponent implements OnInit {
  ...
  getTarnscribedFiles(page){
    ...
    this.dataServices.callAPI("get", {},`client/transcribed/queries?currentPage=${page}&pageSize=${this.pageSize}&module=${this.module}`).pipe(
      take(1),
      map(res => {
  ...
  onProcess(fileKey,fileId){
    if(this.coinsLeft>0){
      this.fileId=fileId
      const body={fileKey,fileId,module:"Queries"}
      this.dataServices.callAPI("post", body,`client/queries/content`).pipe(
        take(1),
        map(res => {
          this.getTarnscribedFiles(this.page);
        })).subscribe()
  ...
  viewFile(fileId){
    this.router.navigate(['/client/finance/audio','queries', fileId]);
  }
  download(file) {
